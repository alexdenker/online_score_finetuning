...
class CondSDE(torch.nn.Module):
    ...
    def forward(self, ts, xT):
        ...
        for t0, t1 in zip(ts[:-1], ts[1:]):
            ...
            g_f = (s_new * s_new.detach()).sum(dim=(1,2,3)).unsqueeze(1)
            f_w = (s_new * dW).sum(dim=(1,2,3)).unsqueeze(1)
            kldiv = kldiv - dt.abs() * f_sq * diffusion.pow(2)[:,None] + math.sqrt(2) * diffusion[:,None] * dt.abs() * g_f + math.sqrt(2) * diffusion[:,None] * f_w
...
with wandb.init(**wandb_kwargs) as run:
    ...
    for i in tqdm(range(cfg["num_steps"])):
        ...
        loss_data = 1/2*1/noise_level**2*torch.sum((forward_op.A(xt[-1]) - cond)**2, dim=(1,2,3))
        loss_kldiv = kldiv
        ...
        wandb.log(
                    {"train/loss_kldiv": loss_kldiv.mean().item(), "step": i}
                ) 
