...
class SDE(torch.nn.Module):
    ...
    def __init__(self, model, sde):
        ...
        self.sde_type = 'stratonovich' #'ito' #"stratonovich" # stratonovich
    # Drift
    def f(self, t, y):
        print(t)
...
bm = torchsde.BrownianInterval(t0=0.0, t1=1.0 - 1.e-3, size=(batch_size, 784), device='cuda', entropy=123, cache_size=None)
with torch.no_grad():
    ys = torchsde.sdeint(sde_model, y0, ts, method='heun', dt = ts[1] - ts[0], bm=bm)
# midpoint  reversible_heun
print(ys.shape)
